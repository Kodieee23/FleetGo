...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between">
    ...
    <div class="mt-4 md:mt-0">
        <a href="{{ route('driver.trips.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-full shadow-sm transition-colors">
            <ion-icon name="add-circle-outline" class="text-xl mr-2"></ion-icon> Log New Trip
        </a>
    </div>
</div>
...
<div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 mb-8">
    <form method="GET" action="{{ route('driver.trips') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="flex items-end space-x-2">
            ...
            <a href="{{ route('driver.trips') }}" class="px-4 py-2.5 bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold rounded-xl shadow-sm transition-colors flex items-center justify-center">
...
<div class="space-y-4">
    @forelse($trips as $trip)
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between hover:shadow-md transition-shadow">
            <div class="flex items-start space-x-4">
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $trip->destination }}</h3>
            ...
            <div class="mt-4 md:mt-0 flex flex-col items-end">
                ...
                @else
                    ...
                    <form action="{{ route('driver.trips.end', $trip->id) }}" method="POST" class="mt-2">
                        @csrf
                        <button type="submit" class="text-sm font-bold text-red-600 dark:text-red-400 hover:underline">End Trip</button>
                    </form>
                @endif
            </div>
        </div>
    @empty
        <div class="bg-white dark:bg-gray-800 rounded-3xl p-12 text-center border border-gray-100 dark:border-gray-700">
            ...
            <p class="text-gray-500 dark:text-gray-400">You haven't logged any trips that match these filters.</p>
        </div>
    @endforelse
</div>
